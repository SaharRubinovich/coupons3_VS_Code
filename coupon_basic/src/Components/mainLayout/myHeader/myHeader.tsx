...
function MyHeader(): JSX.Element {
    ...
    const StyledBadge = styled(Badge)<BadgeProps>(({ theme }) => ({
      ...
      const display = () => {
          if (userType == ''){
              return(<><NavLink to="/login">כניסה למערכת</NavLink></>)
          } else{
              return(<><Button variant="outlined" color="primary" onClick={logout}>התנתקות</Button></>)
          }
      };
      useEffect(()=>{
          display();
        ...
        <div className="myHeader">
            <div style={{position: 'absolute', margin:"20px", marginTop:"30px"}}>
                <IconButton aria-label="cart" onClick={openCart}>
                    <StyledBadge badgeContent={store.getState().cartState.coupons.length} color="secondary">
                        <ShoppingCartIcon />
                    </StyledBadge>
                </IconButton>
            </div>
			<h1>מערכת פאקינג קופונים</h1><br/>
