...
function GetAllCompanies(): JSX.Element {
       ...
       const reduxCompanies: Company[] = useSelector((reduxStore:any)=>{
            return reduxStore.companyState.companies;
       }) 
    ...
    return (
        <div className="getAllCompanies">
			<h1>חברות</h1><hr/>
            {reduxCompanies.map(item => <SignleCompany key={item.id} company={item}/>)}
        </div>
    );
}
